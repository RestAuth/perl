...
RestAuth::Error::BadRequest - Server could not understand the request

=head1 DESCRIPTION

Thrown when the library sends a request that the server could not understand.
This usually means that the server was able to decode the request body
(otherwise L<RestAuth::Error::UnsupportedMediaType> is thrown) but the data
was either missing fields or contained fields that the server does not
understand.

This exception usually hints at an implementation bug in either the library
or the server implementation used.
